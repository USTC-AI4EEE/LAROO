...
import copy
from algos.utils import func_kurtosis,robust_loss
...
class MyRoLatent(object):
    def __init__(self, state_dim, action_dim, latent_dim, max_action, min_v, max_v, replay_buffer, 
                 ...
                 no_noise=True, doubleq_min=0.8,robust_func='huber'):
    ...
    def get_target_q(self, state, actor_net, critic_net, use_noise=False):
        ...
        if use_noise:
            # LAPO 0.1 clip(0.2) -> 0.2 clip(0.5)
    ...
    def train(self, iterations, batch_size=100):
        for it in range(iterations):
            ...
            v_loss = robust_loss(self.robust_func,current_v - target_v.clamp(self.min_v, self.max_v),3.0).mean()
            critic_loss_1 = robust_loss(self.robust_func,current_Q1 - target_Q,3.0).mean()
            critic_loss_2 = robust_loss(self.robust_func,current_Q2 - target_Q,3.0).mean()
    ...
    def load(self, filename, directory):
        ...
        self.critic_optimizer.load_state_dict(torch.load('%s/%s_critic_optimizer.pth' % (directory, filename),map_location=map_location))
        self.critic_target.load_state_dict(torch.load('%s/%s_critic_target.pth' % (directory, filename),map_location=map_location))

        self.actor.load_state_dict(torch.load('%s/%s_actor.pth' % (directory, filename),map_location=map_location))
        self.actor_optimizer.load_state_dict(torch.load('%s/%s_actor_optimizer.pth' % (directory, filename),map_location=map_location))
        self.actor_target.load_state_dict(torch.load('%s/%s_actor_target.pth' % (directory, filename),map_location=map_location))

        self.actor_vae.load_state_dict(torch.load('%s/%s_actor_vae.pth' % (directory, filename),map_location=map_location))
        self.actorvae_optimizer.load_state_dict(torch.load('%s/%s_actor_vae_optimizer.pth' % (directory, filename),map_location=map_location))
        self.actor_vae_target.load_state_dict(torch.load('%s/%s_actor_vae_target.pth' % (directory, filename),map_location=map_location))


class SO2Latent(object):
    def __init__(self, state_dim, action_dim, latent_dim, max_action, min_v, max_v, replay_buffer, 
                 device, discount=0.99, tau=0.005, vae_lr=1e-4, actor_lr=1e-4, critic_lr=5e-4, 
                 max_latent_action=1, expectile=0.8, kl_beta=1.0, 
                 no_noise=True, doubleq_min=0.8,robust_func='huber'):
    ...
    def get_target_q(self, state, actor_net, critic_net, use_noise=True):
        # so2 policy noise=0.3 noise_clip=0.6
        latent_action = actor_net(state)
        if use_noise:
            latent_action += (torch.randn_like(latent_action) * 0.3).clamp(-0.6, 0.6)
        actor_action = self.actor_vae_target.decode(state, z=latent_action)
